...
class MainActivity : Activity() {
    ...
    private fun home(){
        ...
        val titleRow=LinearLayout(this).apply{orientation=LinearLayout.HORIZONTAL;gravity=Gravity.CENTER_VERTICAL};titleRow.addView(neoButton("☰",gold){menu()},LinearLayout.LayoutParams(90,-2));titleRow.addView(t("N E O\nTRADING TRACKER",23f,gold).apply{gravity=Gravity.CENTER;typeface=Typeface.create(Typeface.MONOSPACE,Typeface.BOLD)},LinearLayout.LayoutParams(0,-2,1f));titleRow.addView(t("♢",23f,gold).apply{gravity=Gravity.CENTER},LinearLayout.LayoutParams(90,-2));head.addView(titleRow);head.addView(t("CLASSIFIED MARKET INTELLIGENCE TERMINAL",11f,Color.LTGRAY));head.addView(t("● SECURE DATA LINK // BINANCE USDT",11f,green));head.addView(t("OPERATOR: NEO     CLEARANCE: OMEGA     NODE: 01",10f,Color.rgb(120,155,135)));head.addView(t("SYSTEM // ONLINE     UPLINK // ENCRYPTED     THREAT // NOMINAL",9f,green));body.addView(head);body.addView(spacer(12));val mapPanel=LinearLayout(this).apply{orientation=LinearLayout.VERTICAL;setPadding(12,8,12,12);background=panel(gold)};mapPanel.addView(t("TACTICAL WORLD UPLINK // AZIMUTHAL DISPLAY",10f,gold));mapPanel.addView(GleasonView(this),LinearLayout.LayoutParams(-1,430));body.addView(mapPanel);body.addView(spacer(14))
    ...
    private fun alertCenter(){
        val box=screen("SET ALERT");box.addView(t("CUSTOM PRICE ALERTS",13f,gold));box.addView(t("Choose a coin from Live Markets or Scan Results, then arm a price threshold. Saved alerts remain stored until replaced.",13f,Color.LTGRAY));box.addView(spacer(12));watch.forEach{coin->box.addView(neoButton(coin+" / USDT  //  CREATE ALERT",green){thread{try{val p=JSONObject(URL("https://api.binance.com/api/v3/ticker/price?symbol="+coin+"USDT").readText()).getDouble("price");runOnUiThread{alertDialog(coin,p)}}catch(_:Exception){}}})}
    }

    private fun watchlist(){
        val box=screen("WATCHLIST");val add=neoButton("+ ADD COIN",gold){markets()};box.addView(add);box.addView(spacer(10));watch.forEach{coin->val r=LinearLayout(this).apply{orientation=LinearLayout.HORIZONTAL;gravity=Gravity.CENTER_VERTICAL;background=panel(green);setPadding(12,8,12,8)};r.addView(t("☆  "+coin,16f,gold),LinearLayout.LayoutParams(0,-2,1f));r.addView(t("TRACKING",11f,green));r.setOnClickListener{markets()};box.addView(r,LinearLayout.LayoutParams(-1,-2).apply{setMargins(0,4,0,4)})};box.addView(spacer(14));box.addView(t("WATCHLIST NEWS // INTELLIGENCE FEED",13f,gold));box.addView(t("Market news module reserved for verified live-feed integration.",12f,Color.LTGRAY).apply{background=panel(gold)})
    }
    private fun menu(){
