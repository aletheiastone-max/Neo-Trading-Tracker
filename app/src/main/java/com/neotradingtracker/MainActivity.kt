...
class MainActivity : Activity() {
    ...
    private fun home(){
        ...
        val search=EditText(this).apply{hint="TARGET SYMBOL // e.g. ADA";setTextColor(Color.WHITE);setHintTextColor(Color.rgb(95,130,110));textSize=15f;setPadding(20,18,20,18);background=panel(green)};body.addView(search);search.setOnEditorActionListener{_,_,_->val q=search.text.toString();addCoin(q){ok->if(ok){Toast.makeText(this,"Added "+q.uppercase().removeSuffix("USDT")+" to watchlist",Toast.LENGTH_SHORT).show();home()}else Toast.makeText(this,"Coin not found on Binance USDT",Toast.LENGTH_SHORT).show()};true};val addSearch=neoButton("＋ ADD SYMBOL TO WATCHLIST",gold){val q=search.text.toString();addCoin(q){ok->if(ok){Toast.makeText(this,"Added "+q.uppercase().removeSuffix("USDT"),Toast.LENGTH_SHORT).show();home()}else Toast.makeText(this,"Coin not found on Binance USDT",Toast.LENGTH_SHORT).show()}};body.addView(addSearch);body.addView(spacer(10))
    ...
    private fun bottomNav():LinearLayout{
        ...
        listOf("⌂\nHOME","◎\nSCAN","▥\nMARKETS","☆\nWATCH","♢\nALERTS").forEach{v->val b=t(v,9f,if(v.contains("HOME"))gold else green).apply{gravity=Gravity.CENTER;setOnClickListener{when{v.contains("HOME")->home();v.contains("SCAN")->scan();v.contains("MARKETS")->markets();v.contains("WATCH")->watchlist();else->alertCenter()}}};n.addView(b,LinearLayout.LayoutParams(0,62,1f))}
        return n
    }
    ...
    private fun settings(){val box=screen("SETTINGS");box.addView(sectionLabel("SYSTEM SETTINGS"));val prefs=getSharedPreferences("neo_ui",MODE_PRIVATE);val compact=Switch(this).apply{text="Compact dashboard";setTextColor(Color.WHITE);isChecked=prefs.getBoolean("compact",true);setOnCheckedChangeListener{_,v->prefs.edit().putBoolean("compact",v).apply()}};box.addView(compact);box.addView(t("Market source: Binance USDT\nBackground alert interval: 15 minutes\nWatchlist: persistent",12f,green));box.addView(bottomNav())}
    private fun appearance(){val box=screen("APPEARANCE");box.addView(sectionLabel("DISPLAY"));box.addView(t("NEO MATRIX // GREEN + GOLD",14f,gold));box.addView(t("High contrast tactical theme is active. Animation controls:",12f,Color.LTGRAY));val prefs=getSharedPreferences("neo_ui",MODE_PRIVATE);val anim=Switch(this).apply{text="Matrix animations";setTextColor(Color.WHITE);isChecked=prefs.getBoolean("animations",true);setOnCheckedChangeListener{_,v->prefs.edit().putBoolean("animations",v).apply();Toast.makeText(this@MainActivity,"Applies on next screen",Toast.LENGTH_SHORT).show()}};box.addView(anim);box.addView(bottomNav())}
    private fun connection(){val box=screen("CONNECTION");box.addView(sectionLabel("DATA UPLINK"));box.addView(t("BINANCE PUBLIC MARKET DATA // ACTIVE\nSANJI // EXTERNAL SECURE TRADE LINK\nBYBIT / OKX / COINBASE // QUICK LINKS",12f,green));box.addView(neoButton("TEST MARKET UPLINK",gold){thread{try{JSONObject(URL("https://api.binance.com/api/v3/time").readText());runOnUiThread{Toast.makeText(this,"Market uplink online",Toast.LENGTH_LONG).show()}}catch(_:Exception){runOnUiThread{Toast.makeText(this,"Market uplink unavailable",Toast.LENGTH_LONG).show()}}}});box.addView(bottomNav())}
    private fun tradeLog(){val box=screen("TRADE LOG");box.addView(sectionLabel("OPERATOR LOG"));box.addView(t("Neo does not execute orders itself yet, so no completed exchange orders are stored here. Trades completed in Sanji or an exchange remain in that provider’s history.",12f,Color.LTGRAY));box.addView(bottomNav())}
    private fun help(){val box=screen("HELP");box.addView(sectionLabel("QUICK GUIDE"));box.addView(t("SEARCH: type a Binance base symbol such as ADA or SUI and press Enter.\nWATCHLIST: long-press a market row to add/remove.\nRADAR: dots are live scanned coins; green=ENTER, gold=WAIT, red=NO ACTION. Distance represents the coin’s position in its 24h range.\nTRADE: use Sanji or an exchange link to authenticate and place the order.",12f,Color.WHITE));box.addView(bottomNav())}
    private fun markets(){
