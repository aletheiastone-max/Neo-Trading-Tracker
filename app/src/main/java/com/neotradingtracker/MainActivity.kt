...
class MainActivity : Activity() {
    ...
    private fun panel(stroke:Int=green,fill:Int=Color.argb(235,1,12,8))=GradientDrawable().apply{setColor(fill);cornerRadius=22f;setStroke(2,stroke)}
    private fun neoButton(label:String,accent:Int=green,onClick:()->Unit)=TextView(this).apply{text=label;gravity=Gravity.CENTER;textSize=15f;setTextColor(accent);typeface=Typeface.create(Typeface.MONOSPACE,Typeface.BOLD);setPadding(18,20,18,20);background=panel(accent,Color.argb(245,0,9,6));setOnClickListener{onClick()}}
    ...
    private fun payphone(){
        ...
        val phone=PayphoneView(this).apply{alpha=0f;scaleX=.72f;scaleY=.72f};f.addView(phone,FrameLayout.LayoutParams(-1,-1));setContentView(f)
        phone.animate().alpha(1f).scaleX(1f).scaleY(1f).setDuration(520).withEndAction{Handler(mainLooper).postDelayed({phone.animate().alpha(0f).scaleX(1.32f).scaleY(1.32f).setDuration(430).withEndAction{home()}.start()},1050)}.start()
    }

    private fun home(){
        val f=FrameLayout(this).apply{setBackgroundColor(Color.BLACK)};f.addView(MatrixView(this))
        val scroll=ScrollView(this);body=LinearLayout(this).apply{orientation=LinearLayout.VERTICAL;setPadding(24,30,24,90)}
        val head=LinearLayout(this).apply{orientation=LinearLayout.VERTICAL;setPadding(18,16,18,16);background=panel(gold)}
        head.addView(t("N E O  //  TRADING TRACKER",25f,gold).apply{typeface=Typeface.create(Typeface.MONOSPACE,Typeface.BOLD)});head.addView(t("CLASSIFIED MARKET INTELLIGENCE TERMINAL",11f,Color.LTGRAY));head.addView(t("● SECURE DATA LINK // BINANCE USDT",11f,green));body.addView(head);body.addView(spacer(14))
        ...
        val rp=LinearLayout(this).apply{orientation=LinearLayout.VERTICAL;setPadding(14,12,14,14);background=panel(gold)};rp.addView(t("TACTICAL MARKET RADAR // LIVE SWEEP",12f,gold));rp.addView(RadarView(this),LinearLayout.LayoutParams(-1,500));body.addView(rp);body.addView(spacer(14));body.addView(t("TARGET INTELLIGENCE",13f,gold));results=LinearLayout(this).apply{orientation=LinearLayout.VERTICAL};body.addView(results);scroll.addView(body);f.addView(scroll);setContentView(f);scan()
    }
    ...
    private fun coinCard(c:String,p:Double,ch:Double,s:String,bid:Double){
        ...
        card.addView(t("$"+fmt(p),28f,Color.WHITE).apply{typeface=Typeface.create(Typeface.MONOSPACE,Typeface.BOLD)});card.addView(t("24H VECTOR  "+(if(ch>=0)"▲ " else "▼ ")+"%.2f".format(abs(ch))+"%   //   IDEAL BID  $"+fmt(bid),13f,if(ch>=0)green else Color.rgb(255,90,70)));card.addView(t("ANALYSIS // "+s,11f,accent))
