...
class MainActivity : Activity() {
    ...
    private fun home(){
        ...
        head.addView(t("N E O  //  TRADING TRACKER",25f,gold).apply{typeface=Typeface.create(Typeface.MONOSPACE,Typeface.BOLD)});head.addView(t("CLASSIFIED MARKET INTELLIGENCE TERMINAL",11f,Color.LTGRAY));head.addView(t("● SECURE DATA LINK // BINANCE USDT",11f,green));head.addView(t("OPERATOR: NEO     CLEARANCE: OMEGA     NODE: 01",10f,Color.rgb(120,155,135)));head.addView(t("SYSTEM // ONLINE     UPLINK // ENCRYPTED     THREAT // NOMINAL",9f,green));body.addView(head);body.addView(spacer(12));val mapPanel=LinearLayout(this).apply{orientation=LinearLayout.VERTICAL;setPadding(12,8,12,12);background=panel(gold)};mapPanel.addView(t("TACTICAL WORLD UPLINK // AZIMUTHAL DISPLAY",10f,gold));mapPanel.addView(GleasonView(this),LinearLayout.LayoutParams(-1,430));body.addView(mapPanel);body.addView(spacer(14))
